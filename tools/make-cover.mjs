 ...
 * dwarf drawn deliberately for this purpose, the billionaire looming behind the
 * ridge, and the wordmark big enough to survive a Discord thumbnail.
 *
 ...
 *      of the hero is therefore literally the game's character rig, posed by
 *      hand, so the cover cannot drift away from the in-game look.
 ...
 *      it: sky, moon, ridges, the Musk bust, the lit hero, the wordmark.
 *   4. The head is the point of the exercise. The rig draws a 30px-tall face
 *      because that is all the game ever needs; drawDwarfHead() below throws
 *      that away and draws a modelled one over the top — skull mass, brow
 *      ridge, deep-set eyes with catchlights, a potato nose with a lit plane,
 *      an open roar, a forked beard with strands, and the hat with a brim
 ...
 *      rim. That is what stops the figure reading as flat vector clip-art.
...
/**
 * HEAD_ONLY=/path/out.png renders just the hero's head, ~1400px tall, instead of
 * the card — the only way to actually see what the face is doing, since on the
 * finished 1200x630 it is barely 200px and every feature-collision bug so far
 * has been invisible at that size. Add GUIDES=1 to overlay the anatomical bands.
 *
...
function paint(cfg) {
  ...
  function drawBillionaire(c, x, feet, h) {
    ...
    // Near arm: out and level, pointing at the dwarf across the valley.
    c.lineWidth = p(0.042);
    c.beginPath();
    c.moveTo(p(0.070), p(-0.790));
    c.lineTo(p(0.136), p(-0.694));
    c.lineTo(p(0.216), p(-0.680));
    c.stroke();
    c.lineWidth = p(0.022);
    c.beginPath();
    c.moveTo(p(0.214), p(-0.682));
    c.lineTo(p(0.272), p(-0.686));
    c.stroke();
    ...
    c.strokeStyle = rimAt(0.92);
    c.beginPath();
    c.moveTo(p(-0.014), p(-1.005));
    c.bezierCurveTo(p(0.056), p(-1.005), p(0.082), p(-0.962), p(0.076), p(-0.918));
    c.bezierCurveTo(p(0.073), p(-0.902), p(0.067), p(-0.895), p(0.065), p(-0.890));
    c.bezierCurveTo(p(0.083), p(-0.880), p(0.084), p(-0.867), p(0.067), p(-0.863));
    c.stroke();
    c.lineWidth = p(0.0056);
    c.strokeStyle = rimAt(0.6);
    c.beginPath();
    c.moveTo(p(0.054), p(-0.856));
    c.bezierCurveTo(p(0.058), p(-0.841), p(0.047), p(-0.828), p(0.027), p(-0.826));
    c.stroke();
    ...
    c.strokeStyle = jr;
    c.beginPath();
    c.moveTo(p(0.012), p(-0.812));
    c.lineTo(p(0.088), p(-0.806));
    c.bezierCurveTo(p(0.104), p(-0.680), p(0.098), p(-0.740), p(0.096), p(-0.600));
    c.bezierCurveTo(p(0.094), p(-0.560), p(0.100), p(-0.540), p(0.106), p(-0.480));
    c.stroke();
    c.lineWidth = p(0.005);
    c.strokeStyle = rimAt(0.4);
    c.beginPath();
    c.moveTo(p(0.020), p(-0.806));
    c.lineTo(p(0.048), p(-0.716));
    c.stroke();

    // the pointing arm, lit along its top
    c.lineWidth = p(0.0064);
    c.strokeStyle = rimAt(0.75);
    c.beginPath();
    c.moveTo(p(0.076), p(-0.806));
    c.lineTo(p(0.140), p(-0.712));
    c.lineTo(p(0.272), p(-0.694));
    c.stroke();
  ...
  function drawHero() {
    ...
    /** Joint, in the flipped character-local frame the rig draws in. */
    const jp = (n) => {
      const b = bones.get(n);
      return { x: b.x, y: -b.y };
    };
    /** A point a fraction f along a bone, same frame. */
    const tp = (n, f = 1) => {
      const b = bones.get(n);
  ...
  function paintHeadOnly(cfg) {
    ...
    // Use the card's REAL rig geometry, then magnify. Synthesising a head bone
    // here would be wrong: almost every detail (outline weight, the hat cone,
    // the stud strip) scales off `u` while the face scales off headLen, so a
    // made-up headLen:u ratio silently distorts the very thing being inspected.
    const style = {
      ...MF.getDwarf(cfg.hero).style,
      outfit: 1,
      headSize: cfg.rigHeadSize,
      beardStyle: 'none',
      shades: false,
    ...
    const U = cfg.heroU;
    const u = U * (style.scale || 1);
    const bones = MF.resolvePose(MF.DWARF_SKELETON, HERO_POSE, u);
    const LEN = new Map(MF.DWARF_SKELETON.map((b) => [b.name, b.length]));
    const jp = (n) => {
      const b = bones.get(n);
      return { x: b.x, y: -b.y };
    ...
    // Zoom about the head so it fills the frame at the card's own proportions.
    const hb = jp('head');
    const ht = tp('head');
    const headLen = Math.hypot(ht.x - hb.x, ht.y - hb.y) || u;
    const K = (HH * 0.30) / headLen;
    const ctr = mid(hb, ht, 0.46);
    c.save();
    ...
    if (!cfg.guides) return;

    // Same local frame drawDwarfHead uses, so the bands line up with its X()/Y().
    const hs = cfg.headSize;
    const rx = headLen * 0.5 * hs * K;
    const ry = headLen * 0.54 * hs * K;
    c.save();
    c.translate(HW * 0.5, HH * 0.42);
    const X = (v) => v * rx;
    const Y = (v) => v * ry;

    // Each feature's band: nothing it draws may leave these bounds.
    const bands = [
      ['brow / hat brim', -0.95, -0.42, '#ffd23f'],
      ['eyes', -0.40, 0.20, '#4dd2ff'],
      ['nose', 0.06, 0.52, '#7cff9e'],
      ['mouth + teeth', 0.54, 1.02, '#ff5c8d'],
      ['beard', 1.04, 2.20, '#c08cff'],
    ];
...
window.__ready = true;
  ...
  try {
    ...
    await page.evaluate(paint, { ...CFG, headOnly: HEAD_ONLY !== undefined, guides: !!process.env.GUIDES });
